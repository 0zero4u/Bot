// trader.js
// Version 11.3.0 - FINAL: Replaced faulty client 'cancelAll' with direct API implementation.
...
// --- DEPENDENCIES FOR DIRECT API CALL ---
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v11.3.0) ---`);
    ...
    authenticateWebSocket() {
        ...
        const signature = crypto.createHmac('sha256', this.config.apiSecret).update('GET' + timestamp + '/live').digest('hex');
    ...
    // This function now bypasses the broken client and builds a correct, direct API call.
    async safeCancelAll(productId) {
        ...
        try {
            const signatureString = method + timestamp + path + body;
            ...
            this.logger.info(`[PositionManager] Attempting to cancel all orders for product ${productId} via direct API call.`);
            ...
            if (response.status === 200 && response.data.success) {
                this.logger.info(`[PositionManager] Successfully cancelled all open orders for product ${productId}.`);
            } else {
                throw new Error(`API indicated failure: ${JSON.stringify(response.data)}`);
            }
        } catch (e) {
            const errorMessage = e.response ? JSON.stringify(e.response.data) : e.message;
            this.logger.error(`[PositionManager] A direct call to cancelAllOrders for product ${productId} failed – manual check advised`, { message: errorMessage });
        }
    }
}
