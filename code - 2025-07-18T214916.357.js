...
// Version 5.0.4 - With Definitive Timestamp Fix
...
class DeltaExchangeClient {
  constructor(apiKey, apiSecret, baseUrl) {
    ...
    this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/5.0.4' } });
  }
...
class TradingBot {
  ...
  connectToDeltaWs() {
      ...
      ws.on('open', () => {
          ...
          // Use the correct timestamp for the WebSocket signature as well
          const timestamp = this.deltaClient.getTimestamp();
          const signature = this.deltaClient.generateWsSignature(timestamp);
          ws.send(JSON.stringify({ type: 'auth', payload: { api_key: this.deltaClient.apiKey, timestamp: timestamp, signature: signature }}));
      });
      ws.on('message', (data) => {
