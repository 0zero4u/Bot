...
function utilBodyString(body) { if (!body) return ''; return JSON.stringify(body, null, 0); }
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.5' } }); }
    getTimestamp() { return String(Math.floor(Date.now() / 1000)); }
    generateSignature(method, path, timestamp, query = null, body = null) { const sigData = method + timestamp + path + utilQueryString(query) + utilBodyString(body); return crypto.createHmac('sha256', this.apiSecret).update(sigData, 'utf8').digest('hex'); }
    generateWsSignature(timestamp) { const message = 'GET' + timestamp + '/live'; return crypto.createHmac('sha256', this.apiSecret).update(message, 'utf8').digest('hex'); } // Corrected Path
    async makeRequest(method, path, data = null, query = null) {
        ...
        try { const response = await this.axios({ method, url: path, data: data ? utilBodyString(data) : undefined, params: query, headers }); return utilParseResponse(response); }
    ...
    async setLeverage(productId, leverage) { const payload = { leverage: String(leverage) }; return this.makeRequest('POST', `/v2/products/${productId}/orders/leverage`, payload); }
    async setCancelOnDisconnectTimer(timeoutMs) { const payload = { cancel_after: Math.floor(timeoutMs / 1000) }; return this.makeRequest('POST', '/v2/orders/cancel_after', payload); }
...
class TradingBot {
    constructor() {
        ...
        this.ws = null; this.heartbeatTimer = null;
        this.priceAtLastTrade = null; this.isOrderInProgress = false; this.orderBook = { bids: [], asks: [] };
        this.startTime = new Date(); this.activeOrderClientId = null; this.isPositionOpen = false;
        this.isCoolingDown = false; this.isOrderbookReady = false;
        this.initialize();
    }
    ...
    async initialize() {
        ...
        try {
            ...
            logger.info(`🚀 Bot started. Waiting for L1 orderbook and signals.`);
    ...
    resetHeartbeatTimer() {
        ...
        this.heartbeatTimer = setTimeout(() => {
            ...
            if (this.ws) this.ws.terminate(); // Force close, will trigger 'close' event
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('open', () => {
            ...
            this.ws.send(JSON.stringify({ type: 'auth', payload: { 'api-key': config.deltaApiKey, timestamp, signature } }));
        });

        this.ws.on('message', (data) => {
            const msg = JSON.parse(data.toString());
            if (msg.type === 'auth' && msg.success) {
                logger.info('✅ Delta WebSocket authenticated.');
                logger.info('Enabling heartbeat and subscribing to channels...');
                this.ws.send(JSON.stringify({ type: 'enable_heartbeat' }));
                this.ws.send(JSON.stringify({ type: 'subscribe', payload: { channels: [{ name: 'l1_orderbook', symbols: [config.productSymbol] }, { name: 'orders', symbols: [config.productSymbol] }] } }));
                this.resetHeartbeatTimer(); // Start the heartbeat timer
            } else if (msg.type === 'l1_orderbook') {
                // Corrected field names based on documentation
                this.orderBook.bids = [[msg.best_bid, msg.bid_qty]];
                this.orderBook.asks = [[msg.best_ask, msg.ask_qty]];
                if (!this.isOrderbookReady) {
                    this.isOrderbookReady = true; logger.info('✅ L1 Order book is now synchronized.');
                }
                logger.info(`L1 BBO Received: BID ${msg.best_bid} | ASK ${msg.best_ask}`);
            } else if (msg.type === 'orders') { this.handleOrderUpdate(msg);
            } else if (msg.type === 'heartbeat') { logger.info('❤️ Heartbeat received.'); this.resetHeartbeatTimer();
            } else if (msg.type === 'subscription' && !msg.success) { logger.error('❌ Failed to subscribe:', msg);
        ...
        this.ws.on('close', () => {
            ...
            clearTimeout(this.heartbeatTimer); this.isOrderbookReady = false;
    ...
    handleOrderUpdate(order) {
        if (this.isPositionOpen && order.client_order_id === this.activeOrderClientId && order.state === 'filled' && (order.order_type === 'stop_loss_order' || order.order_type === 'take_profit_order')) {
            logger.info(`✅ POSITION CLOSED: ${order.order_type} for ${this.activeOrderClientId} filled.`);
            this.isPositionOpen = false; this.activeOrderClientId = null; this.startCooldown();
        }
    }

    startCooldown() {
        this.isCoolingDown = true; logger.info(`--- COOLDOWN ${config.cooldownSeconds}s STARTED ---`);
        setTimeout(() => { this.isCoolingDown = false; logger.info(`--- COOLDOWN ENDED ---`); }, config.cooldownSeconds * 1000);
    }
    ...
    async handlePriceMessage(message) {
        if (!this.isOrderbookReady) { logger.warn("Signal received, but L1 order book not synchronized. Ignoring."); return; }
        try {
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            ...
            if (!orderData) { logger.warn("Could not prepare order data. Skipping."); return; }
            const orderResponse = await this.deltaClient.placeOrder(orderData);
            logger.info(`Order placed: ${clientOrderId} (ID: ${orderResponse.id})`);
            this.isPositionOpen = true; this.activeOrderClientId = clientOrderId;
            logger.info(`🔥 POSITION OPENED: ${clientOrderId}`);
            this.priceAtLastTrade = currentPrice; logger.info(`Trade baseline reset to: $${currentPrice.toFixed(2)}`);
        } catch (error) { logger.error('Failed to execute trade:', error.message);
        } finally { this.isOrderInProgress = false; }
    }
    ...
    prepareOrderData(side, clientOrderId) {
        ...
        return { ...baseOrder, order_type: 'bracket_order', limit_price: price.toFixed(4), bracket_take_profit_price: takeProfitPrice.toFixed(4), bracket_stop_loss_price: stopLossPrice.toFixed(4) };
    }
...
try { new TradingBot(); } catch (error) { logger.error("Failed to construct trading bot:", error); process.exit(1); }
