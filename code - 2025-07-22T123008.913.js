// trader.js
// Version 11.5.0 - ADDED REST-based position sync and MODIFIED order logic.
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v11.5.0) ---`);
        this.logger.info(`Strategy: ${this.strategy.getName()}, Product: ${this.config.productSymbol}`);
        await this.syncPositionState(); // Initial state sync
    ...
    handleWebSocketMessage(message) {
        if (message.type === 'success' && message.message === 'Authenticated') {
            ...
            this.startHeartbeat();
            // We now sync state before WS starts, but a re-sync here can be good
    ...
    // MODIFIED: Replaced simple flag-setting with a full REST API sync.
    ...
    // NEW: Method to sync position from REST API
    ...
    handleOrderUpdate(orderUpdate) {
        ...
        if (managedOrder.type === 'main' && previousState !== 'filled' && orderUpdate.state === 'filled') {
            ...
            // MODIFIED: Generalized this check to support strategies that require post-entry brackets.
    ...
    // NEW: Function to place only a Stop-Loss order
    async placeStopLossOrder(mainOrder) {
        ...
        // The SL price was calculated by the strategy, but we don't have it here.
        // We must re-calculate it or, preferably, store it. The strategy has the context.
        // Let's retrieve it from the strategy's last calculation.
        ...
        const slOrder = { 
            ...
            size: mainOrder.size, 
            side, 
            order_type: 'stop_order', // This should be stop_order
            stop_price: slPrice.toString(), 
            //stop_order_type: 'stop_loss_order', // This param might be redundant with order_type: 'stop_order'
            reduce_only: true 
        };
        ...
        try {
            ...
            if (slResponse.result) {
                ...
                // The order will be fully registered via the WebSocket confirmation
            } else {
    ...
    async placeBracketOrders(mainOrder) {
        ...
        const slOrder = { product_id: mainOrder.product_id, size: mainOrder.size, side, order_type: 'stop_order', stop_price: slPrice.toString(), reduce_only: true };
