...
// Version 8.1.0 - Added Time-Based Urgency signal filtering.
...
const config = {
    ...
    urgencyTimeframeMs: parseInt(process.env.URGENCY_TIMEFRAME_MS || '1500'),
...
class TradingBot {
    constructor(botConfig) {
        ...
        this.timeOfLastTradePrice = null;
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v8.1.0 Time-Urgency) ---`);
    ...
    async handleSignalMessage(message) {
        ...
        try {
            ...
            // Set initial price and time anchor if not set
            if (this.priceAtLastTrade === null) {
                this.priceAtLastTrade = currentPrice;
                this.timeOfLastTradePrice = Date.now(); // Set time anchor
                this.logger.info(`Initial baseline price and time anchor set: $${currentPrice.toFixed(2)}`);
                return;
            }

            const priceDifference = Math.abs(currentPrice - this.priceAtLastTrade);
            const timeDifference = Date.now() - this.timeOfLastTradePrice;

            // --- URGENCY CHECK LOGIC ---

            // 1. If the time window has passed, the signal is stale. Reset the anchor.
            if (timeDifference > this.config.urgencyTimeframeMs) {
                this.logger.info(`[UrgencyCheck] Stale signal. Time diff ${timeDifference}ms > ${this.config.urgencyTimeframeMs}ms. Resetting price anchor to $${currentPrice.toFixed(2)}.`);
                this.priceAtLastTrade = currentPrice;
                this.timeOfLastTradePrice = Date.now();
                return; // Wait for the next signal
            }

            // 2. If price moves enough WITHIN the time window, it's an urgent signal.
            if (priceDifference >= this.config.priceThreshold) {
                if (!this.isOrderInProgress && !this.isCoolingDown) {
                    ...
                    if (this.hasOpenPosition) {
                        this.logger.info(`[SignalHandler] Urgent signal received, but ignoring because a position is already open for ${this.config.productSymbol}.`);
                        return;
                    }

                    // --- DELEGATE TO STRATEGY ---
                    // The strategy will handle the trade and update the anchors upon success.
                    await this.strategy.onPriceUpdate(currentPrice, priceDifference);
                }
            }
            // If price difference is not enough, do nothing and wait for more price action within the window.
