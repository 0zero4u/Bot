// trader.js
// Version 9.1.0 - FINAL PRODUCTION VERSION
// Restored robust gatekeeping logic to handleSignalMessage for stable operation.
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v9.1.0) ---`);
        ...
        switch (message.type) {
            ...
            case 'positions':
                if (message.product_symbol === this.config.productSymbol) {
                    ...
                    if (!wasOpen && this.hasOpenPosition) this.logger.warn(`[PositionManager] Found EXISTING position on startup/reconnect: Size=${message.size}`);
    ...
    async handleSignalMessage(message) {
        ...
        try {
            ...
            // --- ROBUST GATEKEEPING LOGIC ---
            // If we have a position, the strategy is responsible for management (e.g., checking for exit).
            if (this.hasOpenPosition) {
                if (this.strategy.onPriceUpdate) await this.strategy.onPriceUpdate(currentPrice, priceDifference);
                return;
            }

            // If we are here, we have NO open position. Check for entry signals.
            if (!this.isOrderInProgress && !this.isCoolingDown) {
                if (priceDifference >= this.config.priceThreshold) {
                    ...
                    if (timeElapsed <= this.config.urgencyTimeframeMs) {
                        ...
                        // Reset the anchor price if the move was too slow
