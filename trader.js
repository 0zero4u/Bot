...
// Version 6.0.12 - Corrected signature generation for DELETE requests.
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.12' } }); }
    getTimestamp() { return String(Math.floor(Date.now() / 1000)); }

    // CORRECTED: Signature generation for DELETE requests.
    generateSignature(method, path, timestamp, query = null, body = null) {
        let queryString = utilQueryString(query);
        const bodyString = utilBodyString(body);

        // Per server error logs, DELETE request signatures do NOT include the query string.
        if (method === 'DELETE') {
            queryString = '';
        }
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.12) ---');
        try {
