...
// Version 6.0.14 - Solved race condition with a state lock while waiting for order resolution.
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.14' } }); }
...
class TradingBot {
    constructor() {
        ...
        this.activeOrderClientId = null; // Our new "soft lock"
        this.isPositionOpen = false; this.isCoolingDown = false; this.isOrderbookReady = false;
        this.initialize();
    }

    async initialize() {
        logger.info('--- Bot Initializing (v6.0.14) ---');
        try {
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('close', () => { logger.warn('Delta WebSocket closed. Reconnecting...'); clearTimeout(this.heartbeatTimer); this.isOrderbookReady = false; this.isPositionOpen = false; this.activeOrderClientId = null; setTimeout(() => this.connectToDeltaWs(), config.reconnectInterval); });
    ...
    handlePositionUpdate(position) {
        ...
        if (this.isPositionOpen && !wasPositionOpen) {
            logger.info(`🔥 POSITION OPENED: Size ${position.size} for ${position.symbol}. Lock remains until position is closed.`);
        } else if (!this.isPositionOpen && wasPositionOpen) {
            logger.info(`✅ POSITION CLOSED for ${position.symbol}. Releasing lock.`);
            this.activeOrderClientId = null; // Release the lock
            this.startCooldown();
        }
    }
    ...
    handleOrderUpdate(order) {
        ...
        if (this.activeOrderClientId && order.client_order_id === this.activeOrderClientId) {
            if (order.state === 'cancelled' || order.state === 'rejected') {
                logger.info(`Pending order ${this.activeOrderClientId} failed or was cancelled. Releasing lock.`);
                this.activeOrderClientId = null; // Release the lock because no position will be opened
            }
        }
    }
    ...
    async handlePriceMessage(message) {
        ...
        try {
            ...
            // CRITICAL NEW CHECK: Do not trade if we are waiting for an order to resolve.
            if (priceDifference >= config.priceThreshold && !this.isOrderInProgress && !this.isPositionOpen && !this.activeOrderClientId && !this.isCoolingDown) {
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            ...
            logger.info(`✅ Order placed successfully: ${clientOrderId}. Locking new trades until order resolves.`);
            
            this.activeOrderClientId = clientOrderId; // SET THE LOCK
            ...
            this.activeOrderClientId = null; // Release lock on failure
        } finally {
