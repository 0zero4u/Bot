// trader.js
// Version 10.2.4 - Added detailed error logging for bracket order placement
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v10.2.4) ---`);
            ...
            if (!this.isOrderInProgress && !this.isCoolingDown) {
                if (priceDifference >= this.config.priceThreshold) {
                    ...
                    if (timeElapsed <= this.config.urgencyTimeframeMs) {
                        ...
                        if (this.strategy.onPrice_update) {
                            await this.strategy.onPrice_update(currentPrice, priceDifference);
                        }
    ...
    handleBracketManagement(orderUpdate) {
        ...
        if (managedOrder.type === 'main' && previousState !== 'filled' && orderUpdate.state === 'filled') {
            this.logger.info(`[OrderManager] Main order ${orderUpdate.id} filled. Attaching child orders based on strategy.`);
            if (this.config.strategy.includes('Bracket')) {
                this.placeBracketOrders(orderUpdate);
            } else if (this.config.strategy.includes('Momentum')) {
    ...
    async placeStopLossOrder(mainOrder) {
        ...
        try {
            this.logger.info(`[OrderManager] Attempting to place Stop-Loss with payload: ${JSON.stringify(slOrder)}`);
            const slResponse = await this.placeOrder(slOrder);

            if (slResponse.result) {
                this.logger.info(`[OrderManager] Placed hard Stop Loss (${slResponse.result.id}) for main order ${mainOrder.id}.`);
                ...
                // <<< ENHANCED LOGGING >>>
                const errorMessage = slResponse.error ? JSON.stringify(slResponse.error) : JSON.stringify(slResponse);
                throw new Error(`API rejected stop-loss order. Reason: ${errorMessage}`);
            }
        } catch (error) {
            this.logger.error('[OrderManager] CRITICAL: Failed to place hard stop loss.', {
                payload: JSON.stringify(slOrder),
                error: error.message
            });
        }
    }
    ...
    async placeBracketOrders(mainOrder) {
        ...
        try {
            ...
            if (tpResponse.result && slResponse.result) {
                ...
                // <<< ENHANCED LOGGING >>>
                const tpError = !tpResponse.result ? (tpResponse.error ? JSON.stringify(tpResponse.error) : JSON.stringify(tpResponse)) : "Success";
                const slError = !slResponse.result ? (slResponse.error ? JSON.stringify(slResponse.error) : JSON.stringify(slResponse)) : "Success";
                throw new Error(`One or both bracket orders failed. TP Reason: ${tpError} | SL Reason: ${slError}`);
            }
        } catch (error) {
            this.logger.error('[OrderManager] CRITICAL: Failed to place bracket orders.', {
                error: error.message
            });
        }
    }
    ...
    async cancelActiveStopLoss(clientOrderId) {
        let mainOrder = null;
        for (const order of this.managedOrders.values()) {
            if (order.clientOrderId === clientOrderId && order.type === 'main') {
                mainOrder = order;
                break;
            }
        }

        if (mainOrder && mainOrder.linkedOrders.size > 0) {
            const slOrderId = Array.from(mainOrder.linkedOrders)[0];
            this.logger.info(`[OrderManager] Trailing stop triggered. Cancelling hard stop-loss order ${slOrderId}.`);
            try {
                await this.client.batchCancelOrders(this.config.productId, [slOrderId]);
                this.managedOrders.delete(slOrderId);
            } catch(error) {
                this.logger.error(`[OrderManager] Failed to cancel hard stop-loss:`, { message: error.message });
            }
        }
    }
}
