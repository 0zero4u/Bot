// trader.js
// Version 10.2.3 - Added dedicated SL placement for MomentumRiderStrategy
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v10.2.3) ---`);
    ...
    handleBracketManagement(orderUpdate) {
        ...
        if (managedOrder.type === 'main' && previousState !== 'filled' && orderUpdate.state === 'filled') {
            ...
            // <<< MODIFIED: Route based on active strategy >>>
    ...
    // <<< ADDED: Function to place only a Stop Loss order >>>
    async placeStopLossOrder(mainOrder) {
        ...
        try {
            ...
            if (slResponse.result) {
                ...
                throw new Error(`Failed to place stop loss order: ${JSON.stringify(slResponse)}`);
            }
        } catch (error) {
            this.logger.error('[OrderManager] CRITICAL: Failed to place hard stop loss. Manual intervention may be required.', { message: error.message });
        }
    }
    ...
    async placeBracketOrders(mainOrder) {
        ...
        try {
            ...
            if (tpResponse.result && slResponse.result) {
                ...
                throw new Error(`Failed to place one or both bracket orders. TP: ${JSON.stringify(tpResponse)}, SL: ${JSON.stringify(slResponse)}`);
            }
        } catch (error) {
    ...
    // <<< ADDED: Function to cancel a specific SL order when the trailing stop is hit >>>
    async cancelActiveStopLoss(clientOrderId) {
        let mainOrder = null;
        // Find the main order by its original clientOrderId
        ...
        if (mainOrder && mainOrder.linkedOrders.size > 0) {
            const slOrderId = Array.from(mainOrder.linkedOrders)[0]; // Assuming only one SL is linked
