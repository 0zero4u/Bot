...
// Version 8.5.1 - Fixed Race Condition in Signal Handling
...
class TradingBot {
    constructor(botConfig) {
        ...
        // State for managing throttled logs and heartbeat
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v8.5.1) ---`);
    ...
    async handleSignalMessage(message) {
        ...
        try {
                ...
                if (priceDifference >= this.config.priceThreshold) {
                    if (this.priceMoveStartTime) {
                        ...
                        if (timeElapsed <= this.config.urgencyTimeframeMs) {
                            // --- FIX: Set the flag *before* any await call to prevent race conditions ---
                            ...
                            // Now the function can safely pause. No other signals will get past the isOrderInProgress check.
            ...
            // Failsafe: Reset the flag if an unexpected error occurs in this block
            this.isOrderInProgress = false;
        }
    }
