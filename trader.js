// trader.js
// Version 11.5.4 - ROBUST error handling in safeCancelAll to prevent crashes.
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v11.5.4) ---`);
        ...
        switch (message.type) {
            ...
            case 'positions':
                if (!this.isStateSynced) {
                    this.logger.info('Initial position snapshot received via WebSocket. State is now fully synchronized.');
    ...
    startCooldown() {
        ...
        setTimeout(() => {
            ...
            this.priceMoveStartTime = null;
            this.logger.info('[StateManager] Cooldown finished. Resetting price reference for next signal.');
    ...
    async forceStateCorrection() {
        this.logger.warn('[StateManager] Forcing state correction via REST API sync due to exchange feedback.');
        await this.syncPositionState();
    }
    ...
    async syncPositionState() {
        ...
        try {
            ...
            if (productPosition && parseFloat(productPosition.size) !== 0) {
                ...
                this.logger.info(`[StateManager] REST Sync: Position found for ${this.config.productSymbol}. Size: ${productPosition.size}.`);
            } else {
                this.hasOpenPosition = false;
                this.logger.info(`[StateManager] REST Sync: No position found for ${this.config.productSymbol}.`);
            }
    ...
    registerPendingOrder(clientOrderId, type = 'main') {
        this.pendingOrders.set(clientOrderId, { type });
        this.logger.info(`[OrderManager] Registered pending ${type} order with clientOrderId: ${clientOrderId}`);
    }

    confirmRegisteredOrder(clientOrderId, orderResult) {
        const pending = this.pendingOrders.get(clientOrderId);
        if (pending) {
            this.registerOrder(orderResult, pending.type, clientOrderId);
    ...
    handleOrderUpdate(orderUpdate) {
        ...
        if (managedOrder.type === 'main' && previousState !== 'filled' && orderUpdate.state === 'filled') {
            ...
            if (this.strategy.hasBracketOrders) {
                this.logger.info(`[OrderManager] Strategy requires bracket orders. Attaching child TP/SL orders.`);
                this.placeBracketOrders(orderUpdate);
            } else if (this.strategy.hasFailSafeStop) {
                this.logger.info(`[OrderManager] Strategy requires a fail-safe SL. Attaching SL order.`);
                this.placeStopLossOrder(orderUpdate);
            }
        }
    ...
    // MODIFIED: Added stop_order_type and stop_trigger_method to the payload.
    async placeStopLossOrder(mainOrder) {
        ...
        if (!slPrice) {
            this.logger.error('[OrderManager] CRITICAL: Cannot place fail-safe SL because lastCalculatedStopLoss is not set in the strategy.');
            this.isOrderInProgress = false;
            return;
        }
        ...
        const slOrder = { 
            ...
            stop_order_type: 'stop_loss_order', // This was the missing critical parameter
            stop_trigger_method: 'last_traded_price', // Explicitly set trigger method
            reduce_only: true 
        };
        const clientOrderId = `sl-${uuidv4()}`;

        try {
            this.logger.info(`[OrderManager] Attempting to place Fail-Safe SL with payload: ${JSON.stringify(slOrder)}`);
            ...
            if (slResponse.result) {
                this.logger.info(`[OrderManager] Placed SL (${slResponse.result.id}) order.`);
            } else {
                 this.cancelPendingOrder(clientOrderId);
                 throw new Error(`SL order failed. Reason: ${JSON.stringify(slResponse)}`);
            }
        } catch (error) {
            this.logger.error('[OrderManager] CRITICAL: Failed to place fail-safe stop-loss order.', { error: error.message });
            this.logger.warn('[OrderManager] Attempting to emergency-exit position due to failed SL placement.');
            await this.strategy.exitPosition();
        } finally {
            this.logger.info('[StateManager] Entry sequence finished. Resetting isOrderInProgress flag.');
            this.isOrderInProgress = false;
        }
    }
    ...
    async placeBracketOrders(mainOrder) {
        ...
        const tpOrder = { product_id: mainOrder.product_id, size: mainOrder.size, side, order_type: 'limit', limit_price: tpPrice.toString(), reduce_only: true, post_only: true };
        const slOrder = { product_id: mainOrder.product_id, size: mainOrder.size, side, order_type: 'stop_order', stop_price: slPrice.toString(), stop_order_type: 'stop_loss_order', stop_trigger_method: 'last_traded_price', reduce_only: true };
    ...
    /**
     * [CORRECTED] Safely attempts to cancel all orders for a product.
     * This function is now robust and will not crash the bot on TypeErrors.
    ...
    async safeCancelAll(productId) {
        try {
            ...
            // This log will now only appear if the call truly succeeds without any error.
            this.logger.info(`[PositionManager] All open orders for product ${productId} cancelled cleanly.`);
        } catch (e) {
            // --- ROBUST ERROR HANDLING (THE FIX) ---
            let errorMessage = e.message; // Default to the basic error message for TypeErrors, etc.
            if (e.response && e.response.data) {
                // If it's a specific network error from the exchange, use the detailed response.
            ...
            // We do not re-throw the error, allowing the exit process to continue with the market order.
        }
    }
}
