...
// Version 6.0.11 - Corrected WebSocket connection and subscription sequence.
...
function validateConfig() {
    ...
    if (required.some(key => !config[key])) { logger.error(`FATAL: Missing required configuration: ${required.filter(key => !config[key]).join(', ')}`); process.exit(1); }
}
validateConfig();
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.11' } }); }
    ...
    async cancelAllOpenOrders(productId) { logger.info(`Cancelling all existing open orders for product ID ${productId}...`); return this.makeRequest('DELETE', '/v2/orders/all', null, { product_id: productId }); }
}
...
class TradingBot {
    constructor() {
        ...
        this.isPositionOpen = false; this.isCoolingDown = false; this.isOrderbookReady = false;
        this.initialize();
    }

    async initialize() {
        logger.info('--- Bot Initializing (v6.0.11) ---');
        try {
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('open', () => {
            ...
            // Step 1: Only send authentication request on open.
            const timestamp = this.deltaClient.getTimestamp();
            const signature = this.deltaClient.generateWsSignature(timestamp);
            this.ws.send(JSON.stringify({ type: 'auth', 'api-key': config.deltaApiKey, 'timestamp': timestamp, 'signature': signature }));
        });

        this.ws.on('message', (data) => {
            const msg = JSON.parse(data.toString());

            if (msg.type !== 'heartbeat') { // Don't reset the timer for heartbeats themselves
                this.resetHeartbeatTimer();
            }

            if (msg.type === 'auth' && msg.success) {
                logger.info('✅ Delta WebSocket authenticated. Subscribing to all channels and enabling heartbeats...');
                
                // Step 2: Subscribe to all channels (public and private) after auth is confirmed.
                ...
                // Step 3: Enable heartbeats after subscriptions are requested.
                this.ws.send(JSON.stringify({ type: 'enable_heartbeat' }));

                // Start the heartbeat timer now that we expect to receive them.
                this.resetHeartbeatTimer(); 
            ...
            } else if (msg.type === 'heartbeat') { logger.info('❤️ Heartbeat received.'); this.resetHeartbeatTimer(); // Reset timer on heartbeat
    ...
    handlePositionUpdate(position) {
        ...
        if (this.isPositionOpen && !wasPositionOpen) { logger.info(`🔥 POSITION OPENED: Size ${position.size} for ${position.symbol}.`); }
        else if (!this.isPositionOpen && wasPositionOpen) { logger.info(`✅ POSITION CLOSED for ${position.symbol}.`); this.startCooldown(); }
    }

    handleOrderUpdate(order) { logger.info(`Order Update: Client ID ${order.client_order_id} | Status: ${order.state} | Type: ${order.order_type}`); }
    ...
    async handlePriceMessage(message) {
        if (!this.isOrderbookReady) { logger.warn("Signal received, but L1 book not synchronized. Ignoring."); return; }
        try {
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            try { const cancelled = await this.deltaClient.cancelAllOpenOrders(config.productId); if (cancelled && cancelled.length > 0) logger.info(`Successfully cancelled ${cancelled.length} lingering order(s).`); }
            catch (cancelError) { logger.error(`❌ CRITICAL: Failed to cancel open orders. Aborting trade.`, cancelError.message); this.isOrderInProgress = false; return; }
    ...
    prepareOrderData(side, clientOrderId) {
        ...
        return { ...baseOrder, order_type: 'limit_order', limit_price: price.toFixed(4), bracket_take_profit_price: takeProfitPrice.toFixed(4), bracket_stop_loss_price: stopLossPrice.toFixed(4) };
    }
