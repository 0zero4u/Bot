...
// Version 8.5.0 - Added WebSocket Heartbeat for Connection Stability
...
class TradingBot {
    constructor(botConfig) {
        ...
        this.heartbeatTimeout = null; // <-- For heartbeat timer
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v8.5.0) ---`);
    ...
    // --- HEARTBEAT IMPLEMENTATION START ---
    enableHeartbeat() {
    ...
    startHeartbeatCheck() {
        clearTimeout(this.heartbeatTimeout); // Clear previous timer
    ...
    async initWebSocket() { 
        ...
        this.ws.on('close', (code, reason) => {
            ...
            clearTimeout(this.heartbeatTimeout); // Stop heartbeat check on disconnect
            this.authenticated = false;
            this.isOrderbookReady = false;
    ...
    handleWebSocketMessage(message) {
        if (message.type === 'success' && message.message === 'Authenticated') {
            ...
            this.enableHeartbeat(); // Enable heartbeat on successful connection
            this.startHeartbeatCheck(); // Start the liveness check
            return;
        }
        switch (message.type) {
            case 'heartbeat':
                this.logger.debug('Heartbeat received from server.');
                this.startHeartbeatCheck(); // Reset the timer
                break;
            case 'orders':
