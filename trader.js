...
// Version 6.0.11 - Corrected WebSocket authentication payload structure.
...
class DeltaExchangeClient {
    ...
    async cancelAllOpenOrders(productId) {
        ...
        return this.makeRequest('DELETE', '/v2/orders/all', null, { product_id: productId });
    }
}
...
class TradingBot {
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('open', () => {
            ...
            // CORRECTED: Authentication credentials MUST be in a nested payload object.
        ...
        this.ws.on('close', () => { logger.warn('Delta WebSocket closed. Reconnecting...'); clearTimeout(this.heartbeatTimer); this.isOrderbookReady = false; this.isPositionOpen = false; setTimeout(() => this.connectToDeltaWs(), config.reconnectInterval); });
    ...
    handleOrderUpdate(order) {
        logger.info(`Order Update: Client ID ${order.client_order_id} | Status: ${order.state} | Type: ${order.order_type}`);
    }
