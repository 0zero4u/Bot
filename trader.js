...
// Version 7.0.6 - Added pre-flight validation to prevent invalid (e.g., negative) bracket prices.
...
class TradingBot {
    ...
    async start() {
        this.logger.info('--- Bot Initializing (v7.0.6) ---');
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            ...
            if (takeProfitPrice <= 0 || stopLossPrice <= 0) {
                this.logger.error('ABORTING TRADE: Calculated bracket price is invalid (<= 0). Your offset is likely too large for the current price.', {
