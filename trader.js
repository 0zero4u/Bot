...
// Version 8.3.0 - Throttled High-Frequency Logging
...
class TradingBot {
    constructor(botConfig) {
        ...
        // NEW: State for managing throttled logs
    ...
    /**
     * Checks if a specific log message, identified by a key, is allowed to be printed.
     * Prevents log spam by enforcing a minimum interval between identical messages.
     * @param {string} key - A unique identifier for the log message.
     * @returns {boolean} - True if logging is allowed, false otherwise.
     */
    canLog(key) {
        const now = Date.now();
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v8.3.0) ---`);
    ...
    async initWebSocket() { /* ... same as before ... */ 
    ...
    authenticateWebSocket() { /* ... same as before ... */
    ...
    subscribeToChannels() { /* ... same as before ... */
    ...
    handleWebSocketMessage(message) { /* ... same as before ... */
    ...
    registerOrder(order, type, parentId) { /* ... same as before ... */
        if (!order || !order.id) {
    ...
    handleOrderUpdate(orderUpdate) { /* ... same as before ... */
    ...
    handleOrderCompletion(completedOrder) { /* ... same as before ... */
    ...
    debouncedBatchCancel(orderIds, delay = 250) { /* ... same as before ... */
    ...
    async placeOrder(orderData) { /* ... same as before ... */
        return await this.client.placeOrder(orderData);
    }

    setupHttpServer() { /* ... same as before ... */
