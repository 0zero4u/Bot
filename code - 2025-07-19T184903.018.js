...
// Version 6.0.9 - Implemented active order cancellation for bracket orders.
    ...
    constructor(apiKey, apiSecret, baseUrl) {
        ...
        this.axios = axios.create({
            ...
            headers: {
                ...
                'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.9'
            }
        });
    }
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.9) ---');
        try {
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('message', (data) => {
            ...
            if (msg.type === 'auth' && msg.success) {
                ...
                logger.info(`L1 BBO: BID ${msg.best_bid} | ASK ${msg.best_ask}`);
    ...
    async handleOrderUpdate(order) {
        if (this.isPositionOpen && order.client_order_id === this.activeOrderClientId && order.state === 'filled' && (order.order_type === 'stop_loss_order' || order.order_type === 'take_profit_order')) {
            ...
            // NEW: Cancel any other remaining live orders for this product to prevent orphans.
    ...
    // NEW: Cancels all open orders for the configured product.
    async cancelAllOpenOrders() {
        ...
        try {
            ...
            if (liveOrders && liveOrders.length > 0) {
                // We only need to provide the order 'id' for cancellation.
    ...
    prepareOrderData(side, clientOrderId) {
        ...
        return {
            ...baseOrder,
            order_type: 'limit_order', // CORRECTED: Use 'limit_order' for bracket orders
