...
// Version 6.0.1 - All known critical issues resolved.
...
function utilBodyString(body) {
    if (!body) return '';
    return JSON.stringify(body, null, 0); // Compact format matching Python's separators=(',', ':')
}
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) {
        ...
        this.axios = axios.create({ 
            ...
            headers: { 
                ...
                'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.1' 
            } 
        });
    }

    // Corrected: Timestamp generation matching Python exactly (seconds, as string)
    getTimestamp() {
        return String(Math.floor(Date.now() / 1000));
    }

    // Corrected: Complete signature generation matching Python
    ...
    // Corrected: WebSocket signature with correct /dapi/ws/v1 path for authentication
    generateWsSignature(timestamp) {
        const message = 'GET' + timestamp + '/dapi/ws/v1'; // Path for WS auth in signature
        return crypto.createHmac('sha256', this.apiSecret)
            .update(message, 'utf8')
            .digest('hex');
    }

    // Corrected: Request method with proper signature and error handling
    async makeRequest(method, path, data = null, query = null) {
        ...
        try {
            const response = await this.axios({
                method,
                url: path,
                data: data ? utilBodyString(data) : undefined, // Use utilBodyString
                params: query,
                headers
            });

            return utilParseResponse(response); // Use utilParseResponse
        } catch (error) {
            logger.error('Delta Exchange API Error:', {
                ...
                response: { 
                    ...
                    headers: error.response?.headers // Add headers for more context if needed
                },
    ...
    // Corrected: setLeverage endpoint and payload
    ...
    // Corrected: setCancelOnDisconnectTimer payload
    async setCancelOnDisconnectTimer(timeoutMs) {
        logger.debug(`Setting/Resetting Cancel-on-Disconnect timer to ${timeoutMs}ms.`);
        const payload = {
            cancel_after: Math.floor(timeoutMs / 1000) // Correct key and value in seconds
        };
...
class TradingBot {
    ...
    async initialize() {
        ...
        try {
            ...
            logger.info(`🚀 Bot started successfully. Waiting for signals.`);
        } catch (error) {
        ...
        ws.on('message', (data) => {
            ...
            if (msg.type === 'auth' && msg.success) {
                ...
                ws.send(JSON.stringify({
                    ...
                    payload: {
                        channels: [
                            { name: 'l2_orderbook', symbols: [config.productSymbol] },
    ...
    prepareOrderData(side, clientOrderId) {
        ...
        if (config.orderPlacementStrategy === 'limit_bbo') {
            ...
            logger.error(`Invalid ORDER_PLACEMENT_STRATEGY: "${config.orderPlacementStrategy}"`);
            return null;
        }
        ...
        return {
            ...
            bracket_take_profit_price: takeProfitPrice.toFixed(4), // Corrected key for bracket order TP
            bracket_stop_loss_price: stopLossPrice.toFixed(4)     // Corrected key for bracket order SL
        };
    }
