/**
 * record-assets.js
 * Records market data from Delta Exchange (trades) and Binance (bookTicker)
 * for multiple assets (XRP, DOGE) for validation testing.
 *
 * Usage: node record-assets.js
 */
...
const BINANCE_WS = 'wss://fstream.binance.com/stream?streams=xrpusdt@bookTicker/dogeusdt@bookTicker';
const DURATION_MS = 120_000;

const DELTA_CSV = 'delta_trades.csv';
const BINANCE_CSV = 'binance_bookticker.csv';
...
function connectDelta() {
    ...
    ws.on('open', () => {
        const subscribe = {
            type: 'subscribe',
            payload: {
                channels: [{ name: 'trades', symbols: ['XRPUSD', 'DOGEUSD'] }]
            }
        };
        ws.send(JSON.stringify(subscribe));
        console.log('[Delta] ✅ Connected. Subscribed to XRPUSD, DOGEUSD trades.');
    });

    ws.on('message', (data) => {
        try {
            const msg = JSON.parse(data);
            // Delta sends trades as direct messages
            if (msg.type === 'trades' && msg.sy) {
                const ts = Math.floor((msg.t || Date.now() * 1000) / 1000);
                const symbol = msg.sy;
                const price = parseFloat(msg.p) || 0;
                const size = parseFloat(msg.s) || 0;
                const buyerRole = msg.r === 't' ? 'taker' : 'maker';
...
function connectBinance() {
    ...
    ws.on('open', () => {
        console.log('[Binance] ✅ Connected. Subscribed to xrpusdt@bookTicker, dogeusdt@bookTicker.');
    });
...
function main() {
    ...
    console.log('🚀 Starting multi-asset data recording for 120 seconds...');
