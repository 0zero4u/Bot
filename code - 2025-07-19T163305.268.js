...
// Version 6.0.16 - Implemented native IOC Time-In-Force for superior order management.
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.16' } }); }
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.16) ---');
        try {
    ...
    handleOrderUpdate(order) {
        ...
        if (this.activeOrderClientId && order.client_order_id === this.activeOrderClientId) {
            if (order.state === 'cancelled' || order.state === 'rejected') {
                logger.info(`Pending order ${this.activeOrderClientId} failed or was cancelled by IOC. Releasing lock.`);
