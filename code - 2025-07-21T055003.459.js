// trader.js
// Version 9.2.0 - FINAL PRODUCTION VERSION
// Added isStateSynced flag to prevent startup race condition.
...
class TradingBot {
    constructor(botConfig) {
        ...
        this.logThrottleTimestamps = new Map(); this.heartbeatTimeout = null;
        this.isStateSynced = false; // <<< NEW: The final state flag
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v9.2.0) ---`);
    ...
    async initWebSocket() { 
        ...
        this.ws.on('close', (code, reason) => {
            ...
            clearTimeout(this.heartbeatTimeout);
            this.authenticated = false; this.isOrderbookReady = false; this.isStateSynced = false; // <<< NEW: Reset sync flag on disconnect
    ...
    handleWebSocketMessage(message) {
        ...
        switch (message.type) {
            ...
            case 'orders':
                if (this.strategy.onOrderUpdate && message.data) message.data.forEach(u => this.strategy.onOrderUpdate(u));
                break;
            case 'positions':
                // --- FIX: The first position message confirms the state is synced ---
                ...
                if (message.product_symbol === this.config.productSymbol) {
                    ...
                    if (!wasOpen && this.hasOpenPosition) this.logger.warn(`[PositionManager] Found EXISTING position: Size=${message.size}`);
                    else if (wasOpen && !this.hasOpenPosition) this.logger.info(`[PositionManager] Position for ${this.config.productSymbol} is now CLOSED.`);
    ...
    async handleSignalMessage(message) {
        // --- FIX: Add the final guard clause ---
        if (!this.isStateSynced || !this.isOrderbookReady || !this.authenticated) {
            if (this.canLog('bot_not_ready')) this.logger.warn("Signal received, but bot is not fully ready (State Synced/Order Book/Auth).");
            return;
        }
        try {
