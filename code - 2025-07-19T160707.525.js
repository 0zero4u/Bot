...
// Version 6.0.13 - Corrected DELETE request payload and signature logic.
...
function utilBodyString(body) { if (!body) return ''; return JSON.stringify(body); } // Use standard JSON.stringify
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.13' } }); }
    getTimestamp() { return String(Math.floor(Date.now() / 1000)); }

    generateSignature(method, path, timestamp, query = null, body = null) {
        // CORRECTED: Signature always includes both query and body strings, per authoritative client example.
    ...
    async cancelAllOpenOrders(productId) {
        ...
        // CORRECTED: Parameters for DELETE requests must be in the body, not query string.
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.13) ---');
        try {
    ...
    handleOrderUpdate(order) {
        logger.info(`Order Update: Client ID ${order.client_order_id} | Status: ${order.state} | Type: ${order.order_type} | Reason: ${order.reason || 'N/A'}`);
    }
