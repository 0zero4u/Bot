...
// Version 6.0.7 - Corrected the authentication payload structure.
    ...
    constructor(apiKey, apiSecret, baseUrl) {
        ...
        this.axios = axios.create({
            ...
            headers: {
                ...
                'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.7'
            }
        });
    }
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.7) ---');
        try {
    ...
    connectToDeltaWs() {
        ...
        this.ws.on('open', () => {
            logger.info(`Delta WebSocket opened. Sending initial setup messages...`);
            this.resetHeartbeatTimer();

            // 1. Send Auth Request with CORRECTED FLAT STRUCTURE
            const timestamp = this.deltaClient.getTimestamp();
            const signature = this.deltaClient.generateWsSignature(timestamp);
            this.ws.send(JSON.stringify({
                type: 'auth',
                'api-key': config.deltaApiKey, // Correct: Top-level
                'timestamp': timestamp,        // Correct: Top-level
                'signature': signature         // Correct: Top-level
            }));

            // 2. Enable Heartbeat
            this.ws.send(JSON.stringify({ type: 'enable_heartbeat' }));

            // 3. Subscribe to PUBLIC channels
            this.ws.send(JSON.stringify({
                type: 'subscribe',
                payload: {
        ...
        this.ws.on('message', (data) => {
            ...
            if (msg.type === 'auth' && msg.success) {
                ...
                this.ws.send(JSON.stringify({
                    type: 'subscribe',
                    payload: {
                        channels: [{ name: 'orders', symbols: [config.productSymbol] }]
                    }
                }));
                ...
                if (msg.success) {
                    logger.info(`✅ Successfully subscribed to ${msg.channel_name}`);
                } else {
    ...
    async handlePriceMessage(message) {
        if (!this.isOrderbookReady) {
            logger.warn("Signal received, but L1 order book not synchronized. Ignoring.");
            return;
        }
        try {
