...
// Version 6.0.10 - Re-architected state management to use the `positions` private feed.
...
class DeltaExchangeClient {
    constructor(apiKey, apiSecret, baseUrl) { this.apiKey = apiKey; this.apiSecret = apiSecret; this.baseUrl = baseUrl; this.axios = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'Content-Type': 'application/json', 'User-Agent': 'Bybit-Delta-Trading-Bot/6.0.10' } }); }
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v6.0.10) ---');
        try {
    ...
    // New primary state handler
    handlePositionUpdate(position) {
        if (position.product_id !== config.productId) return; // Ignore updates for other products
    ...
    // Secondary handler, for logging purposes
