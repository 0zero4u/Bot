...
// Version 7.0.4 - Corrected bracket order registration to be immediate and robust.
...
class TradingBot {
    constructor(botConfig) {
        ...
        this.rateLimiter = { requests: [], maxRequestsPerMinute: 200 };
    ...
    async start() {
        this.logger.info('--- Bot Initializing (v7.0.4) ---');
    ...
    registerOrder(order, type, parentId) {
        if (!order || !order.id) {
            this.logger.warn(`Attempted to register invalid ${type} order.`);
            return;
        }
        const orderInfo = {
            id: order.id,
    ...
    handleOrderUpdate(orderUpdate) {
        const registeredOrder = this.liveOrders.get(orderUpdate.id);
        if (!registeredOrder) return; // Not an order we are tracking.

        this.logger.info(`Order update received for ${registeredOrder.type} order:`, { id: orderUpdate.id, oldState: registeredOrder.state, newState: orderUpdate.state });
    ...
    debouncedBatchCancel(orderIds, delay = 250) {
        const uniqueKey = orderIds.sort().join(',');
        if (this.cleanupTimeouts.has(uniqueKey)) clearTimeout(this.cleanupTimeouts.get(uniqueKey));
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            ...
            if (response.success && response.result) {
                const mainOrder = response.result;
                this.logger.info(`Bracket order placed successfully via parent ${clientOrderId}`);
                
                // --- FIX --- Immediately register all three parts of the bracket order
                this.registerOrder(mainOrder, 'main', clientOrderId);
                this.registerOrder(mainOrder.take_profit_order, 'tp', clientOrderId);
                this.registerOrder(mainOrder.stop_loss_order, 'sl', clientOrderId);
