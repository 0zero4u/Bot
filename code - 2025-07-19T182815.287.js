/**
 * Delta Trader Bot - v4.0 (Advanced, Definitive)
 * 
 * This version merges the robust trading logic from the user's v6.0.8 file
 * with the corrected and clarified client from our previous versions.
 * 
...
class TradingBot {
    ...
    async initialize() {
        logger.info('--- Bot Initializing (v4.0 - Advanced) ---');
    ...
    async executeTrade(side, price) {
        logger.info(`TRADE TRIGGER: Side=${side}, Price=${price}. Preparing atomic bracket order.`);
        const orderData = this._prepareBracketOrder(side);
