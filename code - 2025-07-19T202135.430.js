...
// Version 7.0.5 - Corrected bracket order registration to handle API array response.
...
class TradingBot {
    ...
    async start() {
        this.logger.info('--- Bot Initializing (v7.0.5) ---');
    ...
    async executeTrade(currentPrice, priceDifference) {
        ...
        try {
            ...
            const clientOrderId = uuidv4(); // This is the parent ID for the entire trade
            ...
            // --- FIX --- Handle the API response as an array of created orders
            if (response.success && Array.isArray(response.result)) {
                this.logger.info(`Bracket order placement successful, registering ${response.result.length} orders.`);
                response.result.forEach(order => {
                    let orderType = 'main'; // Default type
                    if (order.order_type === 'take_profit_order') orderType = 'tp';
                    else if (order.order_type === 'stop_loss_order') orderType = 'sl';
                    this.registerOrder(order, orderType, clientOrderId);
                });
