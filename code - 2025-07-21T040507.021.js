...
// Version 8.6.0 - Added onPositionUpdate Hook for Strategy State Synchronization
...
class TradingBot {
    ...
    async start() {
        this.logger.info(`--- Bot Initializing (v8.6.0) ---`);
        ...
        switch (message.type) {
            ...
            case 'positions':
                if (message.product_symbol === this.config.productSymbol) {
                    ...
                    // --- CRITICAL FIX: Pass the position update to the strategy ---
                    // This allows the strategy to reliably sync its internal state.
        ...
        try {
            ...
            if (!this.isOrderInProgress && !this.isCoolingDown) {
                if (this.hasOpenPosition) {
                    // Let the strategy manage the open position, don't enter a new one.
